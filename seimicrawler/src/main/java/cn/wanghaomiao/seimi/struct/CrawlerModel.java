...
public class CrawlerModel {
    ...
    public void startRequest(List<String> urls) {
        String[] startUrls = {};
        if(CollectionUtils.isEmpty(urls)){
            startUrls = instance.startUrls();
        } else {
            startUrls = urls.toArray(urls.toArray(new String[urls.size()]));
        }
        ...
        if (ArrayUtils.isNotEmpty(startUrls)) {
            for (String url : startUrls) {
                ...
                queueInstance.clearRecord(crawlerName);
