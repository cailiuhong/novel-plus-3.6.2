...
public class SeimiCrawlerBootstrapListener implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(ContextRefreshedEvent event) {
        ...
        if (context != null) {
            ...
            workersPool = Executors.newFixedThreadPool(Constants.BASE_THREAD_NUM * Runtime.getRuntime().availableProcessors() * CrawlerCache.getCrawlers().size()/2);
            ...
            for (Map.Entry<String, CrawlerModel> crawlerEntry : CrawlerCache.getCrawlerModelContext().entrySet()) {
                for (int i = 0; i < Constants.BASE_THREAD_NUM * Runtime.getRuntime().availableProcessors()/2; i++) {
            ...
            if (isSpringBoot){
                ...
                if (StringUtils.isBlank(crawlerNames)){
                    ...
                    for (String cn:crawlers){
                        ...
                        crawlerModel.startRequest(new ArrayList<>());
                    }
                }
