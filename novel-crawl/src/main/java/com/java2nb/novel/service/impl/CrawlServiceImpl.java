...
public class CrawlServiceImpl implements CrawlService {
    ...
    @Override
    public void autoCrawlSource(AutoCrawlVo autoCrawlVo) throws IOException {
//        CrawlSingleTask crawlSingleTask = new CrawlSingleTask();
        String url = "";
        if(StringUtils.isBlank(autoCrawlVo.getUrl())){
            url = "https://www.h513.com/json?sortid=sortidstr&page=pageStr";
        }

        url.replace("sortidstr",autoCrawlVo.getSortidstr());
        url.replace("pageStr",autoCrawlVo.getPageStr());

        String body = this.getByHttpClientWithChromeByAutoCrawl(url);
        if(StringUtils.isNotBlank(body)){
            ObjectMapper objectMapper = new ObjectMapper();
            AutoCrawCataListVo autoCrawCataListVo = objectMapper.readValue(body, AutoCrawCataListVo.class);
            if(autoCrawCataListVo != null){
                List<AutoCrawCataVo> autoCrawCataVoList = autoCrawCataListVo.getAutoCrawCataVoList();
                if(autoCrawCataVoList != null && autoCrawCataVoList.size()>0){
                    for (AutoCrawCataVo autoCrawCataVo : autoCrawCataVoList) {
                        System.out.println(autoCrawCataVo.toString());
                    }
                }
            }
        }

//        crawlSingleTaskMapper.insertSelective(crawlSingleTask);
    }
    private static String getByHttpClientWithChromeByAutoCrawl(String url) {
        try {
